...
interface AutosProps {
  ...
  onRestoreChartState?: (state: {
    interval: string;
    indicators: string[];
  }) => void;
}

const Autos: React.FC<AutosProps> = ({
  forceMobileMode,
  compact,
  ...
  const [chartTabs, setChartTabs] = useState<
    { symbol: string; interval: string; indicators: string[] }[]
  >([]);
    ...
    if (sessionId && !sessionId.startsWith("new-chat")) {
      void (async () => {
        try {
          ...
          await agentService.interruptSession(
            sessionId,
            token || undefined,
            walletAddress,
          );
  ...
  useEffect(() => {
    if (activeArtifact?.type === "chart") {
      ...
      const isStateMatching =
        chartState?.symbol === newSymbol ||
        chartState?.symbol === newSymbol.replace("/", "");
      ...
      setChartTabs((prevTabs) => {
        ...
        if (existingTabIndex !== -1) {
          ...
          const indicatorsChanged =
            JSON.stringify(existingTab.indicators) !==
            JSON.stringify(newIndicators);

          if (
            (newInterval && existingTab.interval !== newInterval) ||
            indicatorsChanged
          ) {
      ...
      setActiveArtifact((prev) => {
        ...
        const indicatorsChanged =
          JSON.stringify(currentData.indicators || []) !==
          JSON.stringify(newIndicators);
        if (
          currentData.symbol !== newSymbol ||
          (newInterval && currentData.interval !== newInterval) ||
          indicatorsChanged
        ) {
          const newData = {
            ...currentData,
  ...
  useEffect(() => {
    if (activeArtifact?.type !== "chart") return;
    const data = activeArtifact.data;
    if (!data?.symbol) return;

    setChartTabs((prevTabs) => {
      const existingIndex = prevTabs.findIndex((t) => t.symbol === data.symbol);
      if (existingIndex !== -1) {
        const existing = prevTabs[existingIndex];
        const nextInterval = data.interval || existing.interval || "1D";
        const nextIndicators = data.indicators || existing.indicators || [];
        const indicatorsChanged =
          JSON.stringify(existing.indicators) !==
          JSON.stringify(nextIndicators);

        if (existing.interval === nextInterval && !indicatorsChanged) {
          return prevTabs;
        }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (wsData) {
          const formattedWorkspaces: Workspace[] = wsData.map((w: any) => ({
            ...
            sessions: uniqueSessions
              .filter((s: any) => s.workspace_id === w.id)
              .map((s: any) => ({
                id: s.id,
                title: s.title,
                type: "chat",
        ...
        if (uniqueSessions) {
          const inboxOnly: Session[] = uniqueSessions
            .filter((s: any) => !s.workspace_id)
            .map((s: any) => ({
              id: s.id,
              title: s.title,
              type: "chat",
          ...
          setInboxSessions((prev) => {
            const pendingNewChats = prev.filter((s) =>
              s.id.startsWith("new-chat"),
            );
  ...
  const formatChatError = (error: unknown): string => {
    if (axios.isAxiosError(error)) {
      ...
      // OpenRouter free-tier models can return 404 when "Free model publication"
      // privacy setting is disabled on account.
      if (
        detail.includes("No endpoints found matching your data policy") &&
        detail.toLowerCase().includes("free model publication")
      ) {
        return "Model free-tier diblokir oleh setting OpenRouter (Free model publication). Pilih model non-:free atau aktifkan setting tersebut di akun OpenRouter.";
      }
  ...
  const generateAssistantResponse = async (
    ...
    const isProviderFallbackMessage = (text: string) => {
      ...
      return (
        normalized ===
        "i'm sorry, but i'm unable to continue the conversation right now due to a technical issue. please try again later."
      );
    };
    ...
    const buildThoughtKey = (item: any) => {
      if (typeof item === "string") {
        return `s:${String(item).replace(/\s+/g, " ").trim().toLowerCase()}`;
      }
    ...
    const isRuntimeNoiseThoughtLine = (value: string) => {
      const normalized = String(value || "")
        .replace(/\s+/g, " ")
        .trim();
      if (!normalized) return true;
      if (isRuntimePhaseLine(normalized)) return true;
      ...
      if (
        ...
        lower === "tool call" ||
        lower === "tool observe" ||
        lower === "tool registry ready" ||
        lower === "bootstrap prefetch skipped" ||
        lower === "confidence"
      ) {
        return true;
      }

      if (/^(act|observe)\s*:\s*[a-z_][a-z0-9_]*$/i.test(normalized)) {
        return true;
      }
      if (/^(get|set|add|remove|search|verify)_[a-z0-9_]+$/i.test(normalized)) {
        return true;
      }
      return false;
    };
    ...
    const mergeAssistantThoughts = (incomingThoughts: any[]) => {
      if (!Array.isArray(incomingThoughts) || incomingThoughts.length === 0)
        return;
      flushSync(() => {
        setSessionMessages((prev) => {
          const existing = prev[currentSessionId] || [];
          const updated = existing.map((m) => {
            ...
            for (const item of incomingThoughts) {
              if (typeof item === "string" && isRuntimeNoiseThoughtLine(item))
                continue;
    ...
    const appendThought = (thought: any) => {
      ...
      flushSync(() => {
        setSessionMessages((prev) => {
          ...
          const updated = existing.map((m) => {
            if (m.id !== responseId) return m;
            const nextThoughts = Array.isArray(m.thoughts)
              ? [...m.thoughts]
              : [];
            const key = buildThoughtKey(thought);
            const hasKey = nextThoughts.some(
              (item) => buildThoughtKey(item) === key,
            );
            if (!hasKey) {
    ...
    const appendRuntimePhase = (phase: any) => {
      ...
      // Skip internal/low-level phases
      ...
      if (INTERNAL_PHASES.includes(phaseName.toLowerCase())) return;

      const status = String(phase?.status || "done").trim();
      const detail = String(phase?.detail || "").trim();
      const meta = phase?.meta || {};
      const phaseTool = String(meta?.tool || "").trim();
      const phaseStageRaw = String(meta?.stage || "")
        .trim()
        .toLowerCase();

      // Collapse act/observe into single phase - just show tool name
      const phaseStage = ["act", "observe"].includes(phaseStageRaw)
        ? ""
        : phaseStageRaw;

      const loopRaw = meta?.loop;
      const phaseLoop = Number.isFinite(Number(loopRaw))
        ? Number(loopRaw)
        : null;
      const attemptRaw = meta?.attempt;
      const phaseAttempt = Number.isFinite(Number(attemptRaw))
        ? Number(attemptRaw)
        : null;
      const seqRaw = meta?.seq;
      const phaseSeq = Number.isFinite(Number(seqRaw)) ? Number(seqRaw) : null;
      const normalizedDetail = detail.toLowerCase();
      const isSynthetic = Boolean(meta?.synthetic);

      // Use tool name as primary identity (collapse act/observe)
      const phaseIdentity =
        phaseTool || `${phaseName}|${phaseStage}|${phaseLoop ?? ""}`;
      const phaseStableIdentity =
        phaseTool ||
        `${phaseName}|${phaseStage}|${phaseLoop ?? ""}|${normalizedDetail}`;

      if (status.toLowerCase() === "skipped") return;
      if (normalizedDetail.includes("not triggered for this request")) return;

      // Clean title - just show tool name, not "act: tool" and "observe: tool"
      const baseTitle = phaseTool || detail || phaseName.replace(/_/g, " ");
      ...
      if (phaseStage) {
        contentParts.push(`stage=${phaseStage}`);
      }
      const content = contentParts.join(", ");
      const baseLabel = phaseTool || phaseName || "runtime";
      const toolLabel =
        phaseLoop && phaseLoop > 0 ? `${baseLabel} - L${phaseLoop}` : baseLabel;
      ...
      flushSync(() => {
        setSessionMessages((prev) => {
          ...
          const updated = existing.map((m) => {
            if (m.id !== responseId) return m;
            const currentPhases = Array.isArray(m.runtimePhases)
              ? [...m.runtimePhases]
              : [];
            const incomingPhase = {
              name: phaseName,
              ...
              meta: {
                ...
                stable_identity: phaseStableIdentity,
              },
            };
    ...
    const migrateSession = (newSessionId: string) => {
      ...
      setInboxSessions((prev) =>
        dedupeSessionsById(
          prev.map((s) =>
            s.id === currentSessionId ? { ...s, id: newSessionId } : s,
          ),
        ),
      );
    ...
    try {
      ...
      const apiSessionId = currentSessionId.startsWith("new-chat")
        ? "new-chat"
        : currentSessionId;
      await agentService.chatStream(
        ...
        },
        {
          ...
          onRuntime: (runtime) => {
            if (sawRuntimePhaseEvent) return;
          ...
          onRuntimePhase: (phase) => {
            sawRuntimePhaseEvent = true;
            appendRuntimePhase(phase);
          },
          onDone: (event) => {
            const finalContent = event.content || contentBuffer;
            const finalThoughts = Array.isArray(event.thoughts)
              ? event.thoughts
              : undefined;
  ...
  const handleSendMessage = async (
    ...
    if (
      currentSession &&
      (history.length === 0 || currentSession.title === "New Chat")
    ) {
      const newTitle =
        ...
        setInboxSessions((prev) =>
          prev.map((s) =>
            s.id === currentSessionId
              ? { ...s, title: newTitle, isEditing: false }
              : s,
          ),
        );
      } else {
  ...
  const handleTradeDecisionTrigger = async (
    event: TradeDecisionTriggerEvent,
  ) => {
    const modelId = resolveAutoTriggerModelId(activeSessionId);
    const triggerLabel =
      event.triggerType === "validation" ? "VALIDATION" : "INVALIDATION";
    const actionHint =
  ...
  const handleDeleteSession = async (sessionId: string) => {
    ...
    if (activeSessionId === sessionId) {
      ...
      if (firstInbox) {
        ...
        const firstWsSession = workspaces[0]?.sessions.find(
          (s) => s.id !== sessionId,
        );
  ...
  const handleCreateWorkspace = async (name: string) => {
    ...
    try {
      ...
      const result = await agentService.createWorkspace(
        name.trim(),
        token,
        tempId,
        walletAddress,
  ...
  const handleToggleWorkspaceExpand = async (
    workspaceId: string,
    expanded: boolean,
  ) => {
    // Optimistic
    setWorkspaces((prev) =>
      prev.map((ws) =>
        ws.id === workspaceId ? { ...ws, isExpanded: expanded } : ws,
      ),
    );
    ...
    try {
      const token = await getAccessToken();
      if (token) {
        await agentService.updateWorkspace(
          workspaceId,
  ...
  const handleMoveSessionToWorkspace = async (
    sessionId: string,
    ...
    if (sourceId === targetId) {
      console.log(
        `Move skipped: Source ${sourceId} same as Target ${targetId}`,
      );
      return;
    }

    console.log(
      `Moving session ${sessionId} from ${sourceWorkspaceId} to ${targetWorkspaceId}`,
    );
    ...
    try {
      const token = await getAccessToken();
      if (token) {
        await agentService.moveSession(
          sessionId,
          targetId,
          token,
          walletAddress,
  ...
  const handleUpdateWorkspace = async (workspaceId: string, data: any) => {
    setWorkspaces((prev) =>
      prev.map((ws) => (ws.id === workspaceId ? { ...ws, ...data } : ws)),
    );
    try {
      const token = await getAccessToken();
      if (token) {
  ...
  const handleChartStateChange = (newState: {
    symbol: string;
    timeframe: string;
    indicators: string[];
  }) => {
  ...
  const handleOpenChart = (
    ...
    if (existingTab) {
      const mergedIndicators = mergeIndicators(
        existingTab.indicators || [],
        indicators,
      );
      const nextInterval = timeframe || existingTab.interval || "1D";
      if (
        (indicators &&
          mergedIndicators.length !== existingTab.indicators.length) ||
        (timeframe && nextInterval !== existingTab.interval)
      ) {
        setChartTabs((prev) =>
          prev.map((t) =>
            t.symbol === symbol
              ? { ...t, indicators: mergedIndicators, interval: nextInterval }
              : t,
          ),
        );
      }
  ...
  const handleCloseTab = (e: React.MouseEvent, symbol: string) => {
    ...
    if (newTabs.length === 0) {
      ...
      activeArtifact.data?.symbol === symbol
    ) {
  ...
  return (
    <div
      className={`${styles.layoutContainer} ${compact ? styles.compact : ""} `}
    >
      {!showSettings && (
        <AutosSidebar
      ...
      <div
        className={`${styles.contentArea} ${isMinimized && !activeArtifact ? styles.contentCentered : ""} `}
      ...
      >
        ...
        {activeArtifact ? (
              ...
              >
                {chartTabs.map((tab) => (
                  <div
                    ...
                    style={{
                      background:
                        activeArtifact.data?.symbol === tab.symbol
                          ? "rgba(59, 32, 48, 0.1)"
                          : "transparent",
                      color:
                        activeArtifact.data?.symbol === tab.symbol
          ...
          <div
            className={`${isMinimized ? styles.contentCentered : ""} `}
            style={{
              height: "100%",
              display: "flex",
              flexDirection: "column",
              width: "100%",
              maxWidth: isMinimized ? "1300px" : "none",
              margin: isMinimized ? "0 auto" : "0",
            }}
          >
            {showSettings ? (
