...
const parsePositiveNumber = (raw: string): number | null => {    const parsed = parseFloat(raw);
    if (!Number.isFinite(parsed) || parsed <= 0) return null;
    return parsed;
};
const formatInputPrice = (value: number): string => {    if (!Number.isFinite(value) || value <= 0) return '';
    return value < 10 && value > 0.0001 ? value.toFixed(4) : value.toFixed(2);
};
const toNumber = (raw: string): number | null => {    const val = parseFloat(String(raw || '').trim());
    if (!Number.isFinite(val) || val <= 0) return null;
    return val;
};
const computeTpslPrice = (    side: 'buy' | 'sell',    basisPrice: number,    mode: 'tp' | 'sl',    unit: '%' | '$',    inputRaw: string): number | null => {    const input = toNumber(inputRaw);
    if (!input || !Number.isFinite(basisPrice) || basisPrice <= 0) return null;
    if (unit === '%') {        const ratio = input / 100;
        if (mode === 'tp') {            return side === 'buy' ? basisPrice * (1 + ratio) : basisPrice * (1 - ratio);
        }        return side === 'buy' ? basisPrice * (1 - ratio) : basisPrice * (1 + ratio);
    }    if (mode === 'tp') {        return side === 'buy' ? basisPrice + input : basisPrice - input;
...
const OrderForm: React.FC = () => {    // Stop orders disabled for now (requires keeper/executor on-chain; simulation stop-limit is incomplete).
    ...
    const checkSetup = React.useCallback(async () => {        if (authenticated && walletAddress) {            console.log('[OrderForm] Checking trading setup...');
            try {                const status = await onchainService.checkTradingSetup(walletAddress);
    ...
    React.useEffect(() => {        if (!isTradingSetupOpen) {            checkSetup();
        }    }, [isTradingSetupOpen, checkSetup]);
    // Check Session Validity
    React.useEffect(() => {        const checkSession = () => {            const expires = localStorage.getItem('osmo_session_expires');
            const key = localStorage.getItem('osmo_session_key');
            if (key && expires) {                if (new Date(expires) < new Date()) {                    console.log('[OrderForm] Session expired locally');
 ...
 // Check every second
    ...
    const handleQuickPrice = (type: string) => {        if (!selectedMarket?.price) return;
        const currentPrice = selectedMarket.price;
        let targetPrice = currentPrice;
        switch (type) {            case 'Fill':            case 'Mid':            case 'Bid':                targetPrice = currentPrice;
                break;
 ...
 setMarginModeDropdownOpen(false);
                                ...
                                })()}                            </span>                        </div>                        <div className={styles.summaryRow}>                            <span className={styles.summaryLabelDotted}>Position Margin</span>                            <span className={styles.summaryValue}>                                {(authenticated && (summary?.account_value || 0) > 0) && (                                    <span style={{ color: '#A77590' }}>                                        {marginMode} {leverage}x ➝                                    </span>                                )}                                {(() => {                                    if (!authenticated || (summary?.account_value || 0) <= 0) return ' —';
                                ...
                                })()}                            </span>                        </div>                        <div className={styles.summaryRow}>                            <span className={styles.summaryLabelDotted}>Fee (Est)</span>                            <span className={styles.summaryValue}>                                {(() => {                                    if (!authenticated || (summary?.account_value || 0) <= 0) return '—';
                                    const val = parseFloat(amount || '0');
                                    const executePrice = effectiveDisplayPrice;
                                    const notional = isAmountUSD ? val : val * executePrice;
                                ...
                                })()}                            </span>                        </div>                        <div className={styles.summaryRow}>                            <span className={styles.summaryLabelDotted} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>                                Rewards <span style={{ fontSize: '10px', background: '#3A2530', padding: '0 4px', borderRadius: '4px' }}>x</span>                                <span style={{ fontSize: '10px', color: '#5D5FEF', background: 'rgba(93, 95, 239, 0.1)', padding: '0 4px', borderRadius: '4px' }}>New</span>                            </span>                            <span className={styles.summaryValue}>                                {(() => {                                    if (!authenticated || (summary?.account_value || 0) <= 0) return '—';
                                ...
                                })()}                            </span>                        </div>                    </div>                )}                <button                    className={styles.mainActionBtn}                    onClick={() => {                        if (!authenticated) {                            handleConnect();
