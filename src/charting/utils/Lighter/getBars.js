...
export const getBars = async (
    ...
    try {
        ...
        const url = `${API_ORIGIN}/api/candles/${encodeURIComponent(symbol)}?exchange=lighter&limit=${limit}&resolution=${encodeURIComponent(resolution)}`;
        ...
        console.error('[Lighter getBars] Error:', err);
        onErrorCallback(err);
    }
};
...
export const subscribeBars = (
    ...
    const symbol = symbolInfo.name.replace('/', '-');
    const wsUrl = `${WS_ORIGIN}/ws/lighter/${encodeURIComponent(symbol)}`;
    ...
    ws.onmessage = (event) => {
        try {
            ...
            console.error('[Lighter subscribeBars WS] Error parsing message:', e);
        }
    };
