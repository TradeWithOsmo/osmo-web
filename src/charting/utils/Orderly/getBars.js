...
export const getBars = async (
    ...
    try {
        ...
        const url = `${API_ORIGIN}/api/candles/${encodeURIComponent(symbol)}?exchange=orderly&limit=${limit}&resolution=${encodeURIComponent(resolution)}`;

        console.log(`[Orderly getBars]: Fetching ${url}`);
        ...
        console.error('[Orderly getBars]: ❌ Error:', err.message);
        onErrorCallback(err);
    }
};
...
export const subscribeBars = (
    ...
    // Connect to Orderly-specific trade/price stream
    const wsUrl = `${WS_ORIGIN}/ws/orderly/${encodeURIComponent(symbol)}`;
    ...
    ws.onmessage = (event) => {
        try {
            ...
            console.error('[Orderly subscribeBars]: WS error', e);
        }
    };
