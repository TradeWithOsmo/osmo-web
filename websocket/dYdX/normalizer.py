"""Normalize dYdX v4 price data to unified schema"""
from typing import Any, Dict
from datetime import datetime
from services.canonical_source_registry import canonical_registry


def normalize_dydx_prices(data: Any) -> Dict[str, Any]:
    ...
    for item in items:
        ...
        normalized[symbol] = {
            "symbol": symbol,
            "price": str(item.get("price") or item.get("oracle_price") or 0),
            "timestamp": ts,
            "source": "dydx",
            "category": canonical_registry.get_category_sync(symbol),
            "funding_rate": item.get("next_funding_rate") or 0,
